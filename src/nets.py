...
class DenseLayer(nn.Module):
    ...
    # k  - the growth rate
    # l  - the current layer
    def __init__(self, input_features, k, l):
...
class DenseBlock(nn.Module):
    ...
    def __init__(self, input_features, k, number_of_layers):
        super(DenseBlock, self).__init__()
        self.number_of_layers = number_of_layers
        l = 0
        for i in range(number_of_layers):
            l += 1
            dense_layer = DenseLayer(input_features, k, l)
            self.add_module('DenseLayer' + str(i), dense_layer)
        self.output_features = input_features + k * number_of_layers
...
class DenseNet(nn.Module):
    ...
    def __init__(self, config, k0=40):
        ...
        for i in range(0, self.num_blocks):
            dense_block = DenseBlock(features, k, self.num_layers)
            self.add_module('dense_block' + str(i), dense_block)
            features = dense_block.output_features


        self.theta = nn.Conv1d(features, 72, 1)
